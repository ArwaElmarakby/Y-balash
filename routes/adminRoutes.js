...
router.get('/reports/users', authMiddleware, adminMiddleware, adminController.getUserGrowthReport);
router.get('/reports/products', authMiddleware, adminMiddleware, adminController.getProductReport);
router.get('/reports/orders', authMiddleware, adminMiddleware, adminController.getOrderReport);
router.get('/reports/combined', authMiddleware, adminMiddleware, adminController.getCombinedReport);


router.get('/reports/revenue/pdf', authMiddleware, adminMiddleware, async (req, res) => {
    try {
        const response = await adminController.getRevenueReport(req, res, true);
        generatePDF(response.pdfData, (err, buffer) => {
            if (err) {
...
router.get('/reports/users/pdf', authMiddleware, adminMiddleware, async (req, res) => {
    try {
        const response = await adminController.getUserGrowthReport(req, res, true);
        generatePDF(response.pdfData, (err, buffer) => {
            if (err) {
                return res.status(500).json({ message: 'Error generating PDF' });
            }
            res.setHeader('Content-Type', 'application/pdf');
            res.setHeader('Content-Disposition', 'attachment; filename=user-growth-report.pdf');
            res.send(buffer);
        });
    } catch (error) {
        res.status(500).json({ message: 'Server error', error });
...
router.get('/reports/products/pdf', authMiddleware, adminMiddleware, async (req, res) => {
    try {
        const response = await adminController.getProductReport(req, res, true);
        generatePDF(response.pdfData, (err, buffer) => {
            if (err) {
                return res.status(500).json({ message: 'Error generating PDF' });
            }
...
router.get('/reports/orders/pdf', authMiddleware, adminMiddleware, async (req, res) => {
    try {
        const response = await adminController.getOrderReport(req, res, true);
        generatePDF(response.pdfData, (err, buffer) => {
            if (err) {
                return res.status(500).json({ message: 'Error generating PDF' });
            }
...
router.get('/reports/combined/pdf', authMiddleware, adminMiddleware, async (req, res) => {
    try {
        const response = await adminController.getCombinedReport(req, res);
        
        // Create a combined PDF data structure
        const pdfData = {
            title: 'Combined Analytics Report',
            date: new Date().toLocaleDateString(),
            sections: [
                {
                    title: 'Revenue Report',
                    data: [
                        { label: 'Current Month Revenue', value: `${response.report.revenue.currentMonth.revenue.toFixed(2)} EGP` },
                        { label: 'Last Month Revenue', value: `${response.report.revenue.lastMonth.revenue.toFixed(2)} EGP` },
                        { label: 'Percentage Change', value: response.report.revenue.percentageChange }
                    ]
                },
                {
                    title: 'User Growth Report',
                    data: [
                        { label: 'Current Month New Users', value: response.report.users.currentMonth.newUsers },
                        { label: 'Last Month New Users', value: response.report.users.lastMonth.newUsers },
                        { label: 'Percentage Change', value: response.report.users.percentageChange },
                        { label: 'Total Users', value: response.report.users.totalUsers }
                    ]
                },
                {
                    title: 'Product Sales Report',
                    data: [
                        { label: 'Current Month Items Sold', value: response.report.products.currentMonth.totalItemsSold },
                        { label: 'Last Month Items Sold', value: response.report.products.lastMonth.totalItemsSold },
                        { label: 'Items Sold Change', value: response.report.products.percentageChanges.itemsSold },
                        { label: 'Current Month Revenue', value: `${response.report.products.currentMonth.totalRevenue.toFixed(2)} EGP` },
                        { label: 'Last Month Revenue', value: `${response.report.products.lastMonth.totalRevenue.toFixed(2)} EGP` },
                        { label: 'Revenue Change', value: response.report.products.percentageChanges.revenue }
                    ]
                },
                {
                    title: 'Order Report',
                    data: [
                        { label: 'Current Month Total Orders', value: response.report.orders.currentMonth.orders.total.count },
                        { label: 'Last Month Total Orders', value: response.report.orders.lastMonth.orders.total.count },
                        { label: 'Orders Change', value: response.report.orders.percentageChanges.totalCount },
                        { label: 'Current Month Order Value', value: `${response.report.orders.currentMonth.orders.total.totalAmount.toFixed(2)} EGP` },
                        { label: 'Last Month Order Value', value: `${response.report.orders.lastMonth.orders.total.totalAmount.toFixed(2)} EGP` },
                        { label: 'Order Value Change', value: response.report.orders.percentageChanges.totalAmount }
                    ]
                }
            ]
        };
        ...
        generatePDF(pdfData, (err, buffer) => {
            if (err) {
                return res.status(500).json({ message: 'Error generating PDF' });
            }
            res.setHeader('Content-Type', 'application/pdf');
