...
router.put('/withdrawal-requests/:requestId/approve', authMiddleware, adminMiddleware, async (req, res) => {
  try {
...
// في routes/adminRoutes.js
router.put('/withdrawal-requests/:requestId/reject', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    const { requestId } = req.params;
    const { notes } = req.body;
...
router.get('/withdrawal-requests', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    ...
    const restaurants = await Restaurant.find(filter)
      .select('name withdrawalRequests balance')
      .populate('managedBy', 'email');

    const allRequests = restaurants.reduce((acc, restaurant) => {
      const requests = restaurant.withdrawalRequests.map(request => ({
        ...request.toObject(),
        restaurantName: restaurant.name,
        restaurantId: restaurant._id,
        sellerEmail: restaurant.managedBy?.email,
    ...
    res.status(200).json({
      ...
      requests: allRequests.sort((a, b) => b.requestedAt - a.requestedAt)
    });
  } catch (error) {
    res.status(500).json({ message: 'Server error', error: error.message });
  }
