...
router.post('/withdrawal-request', authMiddleware, sellerMiddleware, async (req, res) => {
  try {
    ...
    res.status(201).json({
      ...
      request: restaurant.withdrawalRequests[restaurant.withdrawalRequests.length - 1]
    });
  } catch (error) {
    res.status(500).json({ message: 'Server error', error: error.message });
  }
...
router.get('/withdrawal-requests', authMiddleware, sellerMiddleware, async (req, res) => {
  try {
    const seller = req.user;
    const restaurant = await Restaurant.findById(seller.managedRestaurant)
    ...
    res.status(200).json({
      balance: restaurant.balance,
      requests: restaurant.withdrawalRequests.sort((a, b) => b.requestedAt - a.requestedAt)
    });
  } catch (error) {
    res.status(500).json({ message: 'Server error', error: error.message });
  }
