...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      // 1. Validate Order exists and belongs to the seller's restaurant
      const order = await Order.findOne({
        _id: orderId,
        restaurantId: seller.managedRestaurant,
      })
        .populate('userId', 'email name phone') // Client info
        .populate('items.itemId', 'name price imageUrl'); // Ordered items
