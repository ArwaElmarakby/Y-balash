...
exports.applyPointsDiscount = async (req, res) => {
    ...
    try {
        ...
        // 4. Apply points discount if requested
        let pointsDiscount = 0;
        let pointsUsed = 0;
        let pointsDiscountPercentage = 0;

        if (usePoints && user.points > 0) {
            // Calculate discount (example: 10 points = 2 EGP discount)
            const discountRate = 2; // 2 EGP per 10 points
            pointsUsed = Math.min(user.points, Math.floor(totalPrice / discountRate) * 10);
            pointsDiscount = (pointsUsed / 10) * discountRate;
            pointsDiscountPercentage = (pointsDiscount / totalPrice) * 100;
        ...
        res.status(200).json({
            success: true,
            originalTotal: (totalPrice + pointsDiscount).toFixed(2),
            totalPrice: totalPrice.toFixed(2),
            pointsUsed,
            pointsDiscount: pointsDiscount.toFixed(2),
            pointsDiscountPercentage: pointsDiscountPercentage.toFixed(2) + '%',
