...
exports.getMonthlyEarningsPercentage = async (req, res) => {
    try {
        const seller = req.user;
        ...
        if (last > 0) {
            const change = ((current - last) / last) * 100;
            percentageChange = (change >= 0 ? "+" : "") + Math.round(change) + "%";
        } else if (current > 0) {
            percentageChange = "+100%";
        }
        ...
        console.error("Error in getMonthlyEarningsPercentage:", error);
        res.status(500).json({
            success: false,
