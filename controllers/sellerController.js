...
exports.getMonthlyEarnings = async (req, res) => {
    try {
        ...
        // استعلامات متوازية للحصول على البيانات
        ...
        const currentEarnings = currentMonthResult[0]?.total || 0;
        const lastEarnings = lastMonthResult[0]?.total || 0;

        // حساب النسبة المئوية للتغير
        let percentage = 0;
        if (lastEarnings > 0) {
            percentage = ((currentEarnings - lastEarnings) / lastEarnings) * 100;
        } else if (currentEarnings > 0) {
            percentage = 100;
        }

        // تنسيق النسبة مع إضافة علامة % و+/- حسب الحالة
