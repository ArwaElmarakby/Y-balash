...
exports.confirmCashPayment = async (req, res) => {
    ...
    try {
        ...
        // Calculate points based on the final total (which already includes any points discount)
        const pointsToAdd = Math.floor(order.totalAmount / 40) * 5;
        ...
        res.status(200).json({ 
            ...
            order: {
                id: order._id,
                status: order.status,
                totalAmount: order.totalAmount // This should reflect the discounted amount
            },
            pointsAdded: pointsToAdd
        });
