...
exports.usePointsForDiscount = async (req, res) => {
    ...
    try {
        ...
        // تحديث حالة استخدام النقاط في الكارت
        cart.usePoints = usePoints;
        cart.pointsUsed = 0; // إعادة تعيين
        ...
        if (usePoints && user.points >= 10) {
            ...
            discountFromPoints = possibleDiscounts * 3;
            pointsUsed = possibleDiscounts * 10;
            cart.pointsUsed = pointsUsed; // حفظ النقاط المستخدمة في الكارت
        }

        await cart.save();

        // Calculate cart totals بنفس الطريقة في cashPayment
        let totalItemsPrice = 0;
        cart.items.forEach(item => {
            totalItemsPrice += item.quantity * parseFloat(item.itemId.price);
        });
        ...
        res.status(200).json({
            ...
            pointsUsed,
            discountFromPoints,
            remainingPoints: user.points - pointsUsed, // يعكس الرصيد بعد الخصم
            cartSummary: {
                totalItems: cart.items.length,
                totalOffers: cart.offers.length,
                totalItemsPrice: totalItemsPrice.toFixed(2),
