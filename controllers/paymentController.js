...
exports.cashPayment = async (req, res) => {
    ...
    try {
        ...
        // Calculate totals (same logic as usePointsForDiscount)
        let totalItemsPrice = 0;
        cart.items.forEach(item => {
            totalItemsPrice += item.quantity * parseFloat(item.itemId.price);
        });
        ...
        if (req.body.usePoints && user.points >= 10) {
        ...
        // Restaurant ID logic remains the same
        let restaurantId = null;
        if (cart.items.length > 0) {
            restaurantId = cart.items[0].itemId.restaurant || null;
        } 
        ...
        const order = new Order({
            ...
            totalAmount: totalPrice, // Use the calculated total
            status: 'pending',
            paymentMethod: 'cash',
            pointsUsed: pointsUsed // Store points used if any
        });
