...
exports.cashPayment = async (req, res) => {
    const userId = req.user.id;
    const { usePoints } = req.body; // نضيف هذا البارامتر
    ...
    try {
        ...
        // Calculate totals (بنفس الطريقة بالضبط)
        let totalItemsPrice = 0;
        cart.items.forEach(item => {
            totalItemsPrice += item.quantity * parseFloat(item.itemId.price);
        });
        ...
        // تطبيق خصم النقاط إذا كان usePoints = true
        ...
        const order = new Order({
            ...
            status: 'pending',
            paymentMethod: 'cash',
            pointsUsed: pointsUsed, // إضافة PointsUsed إلى Order
        ...
        await createNotification(
            ...
            'new_order',
            'New Order Received',
            `New cash order #${order._id} for ${totalPrice} EGP (Discount: ${discountFromPoints} EGP)`,
            order._id
        );
        ...
        res.status(200).json({ 
            ...
            orderId: order._id,
            totalAmount: totalPrice, // المبلغ بعد الخصم
