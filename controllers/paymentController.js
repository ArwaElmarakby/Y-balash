...
exports.cashPayment = async (req, res) => {
    const userId = req.user.id;
    const { usePoints = false } = req.body; // قيمة افتراضية false إذا لم تُرسل
    ...
    try {
        ...
        if (!cart || (!cart.items.length && !cart.offers.length)) {
        ...
        const calculateTotal = (array, key) => {
            return array.reduce((sum, item) => sum + (item.quantity * parseFloat(item[key].price)), 0);
        };

        const totalItemsPrice = calculateTotal(cart.items, 'itemId');
        const totalOffersPrice = calculateTotal(cart.offers, 'offerId');
        const shippingCost = 50;
        const importCharges = (totalItemsPrice + totalOffersPrice) * 0.1;
