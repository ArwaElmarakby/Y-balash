...
exports.cashPayment = async (req, res) => {
    ...
    try {
        ...
        // Check if user used points
        let pointsUsed = 0;
        let discountFromPoints = 0;
        const user = await User.findById(userId);
        
        if (user.points >= 10) {
            // Calculate possible discount from points (10 points = 3 EGP)
        ...
        const order = new Order({
            ...
            status: 'pending',
            paymentMethod: 'cash',
            pointsUsed: pointsUsed, // Store points used in order
            discountFromPoints: discountFromPoints // Store discount from points
        });
