...
exports.cashPayment = async (req, res) => {
    ...
    try {
        ...
        // Calculate total price before points discount
        let totalPriceBeforePoints = totalItemsPrice + totalOffersPrice + shippingCost + importCharges;
        ...
        if (user.points >= 10) {
            ...
            pointsDiscount = possibleDiscounts * 3; // 10 points = 3 EGP
            pointsUsed = possibleDiscounts * 10;
        }

        // Calculate final total after points discount
        const finalTotal = Math.max(0, totalPriceBeforePoints - pointsDiscount);

        // Create an order with the final total
        const order = new Order({
            userId: userId,
            restaurantId: restaurantId,
            items: cart.items.map(item => ({
            ...
            originalTotal: totalPriceBeforePoints
        });
        ...
        await createNotification(
            ...
            'new_order',
            'New Order Received',
            `New cash order #${order._id} for ${finalTotal} EGP`,
            order._id
        );
