...
exports.cashPayment = async (req, res) => {
    ...
    try {
        ...
        const discountFromPoints = user.points >= 10 ? Math.floor(user.points / 10) * 3 : 0; // 10 نقاط = 3 جنيه
        ...
        const order = new Order({
            ...
            totalAmount: finalPrice, // استخدم السعر النهائي هنا
            status: 'pending',
            paymentMethod: 'cash' 
        });
        ...
        res.status(200).json({ 
            ...
            orderId: order._id,
            totalAmount: finalPrice // أرسل السعر النهائي
        });
    } catch (error) {
        res.status(500).json({ message: 'Cash payment failed', error: error.message });
    }
