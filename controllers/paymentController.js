...
exports.cashPayment = async (req, res) => {
    ...
    try {
        ...
        // Calculate discount from points if any
        ...
        // Total price calculation (after points discount)
        const totalPrice = totalItemsPrice + totalOffersPrice + shippingCost + importCharges - discountFromPoints;

        // Create an order with the final price (after discount)
        const order = new Order({
            userId: userId,
            restaurantId: restaurantId,
            items: cart.items.map(item => ({
            ...
            status: 'pending',
            paymentMethod: 'cash',
            pointsUsed: discountFromPoints > 0 ? Math.floor(discountFromPoints / 3) * 10 : 0,
        ...
        res.status(200).json({ 
            ...
            orderId: order._id,
            orderDetails: {
                subtotal: totalItemsPrice + totalOffersPrice,
